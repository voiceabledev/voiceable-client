...
const SUGGESTED_BEHAVIORS = [
  ...
  { label: "Lead Qualification", icon: FiTrendingUp, prompt: "Build a lead qualification specialist that qualifies prospects, collects contact information, and schedules follow-up meetings." },
...
export default function AssistantsList() {
  ...
  const handleAssistantClick = (assistant: Agent) => {
    ...
    router.push(`/assistants/${identifier}`);
  };
  ...
  const handleCreateFromPrompt = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      stashCreateAgentWizardState(initialData);
      router.push("/assistants/create?step=1");
    } catch (error) {
