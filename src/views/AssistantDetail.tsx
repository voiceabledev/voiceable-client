...
import { consumeCreateAgentWizardState } from "@/lib/create-agent-wizard-state";
...
export default function AssistantDetail() {
  ...
  const [wizardLocationState, setWizardLocationState] = useState<
    ReturnType<typeof consumeCreateAgentWizardState>
  >(null);
  const wizardBootstrapped = useRef(false);
  useEffect(() => {
    const creating = pathname === "/assistants/create" || id === "create";
    if (creating && !wizardBootstrapped.current) {
      wizardBootstrapped.current = true;
      setWizardLocationState(consumeCreateAgentWizardState());
    }
  }, [pathname, id]);
  ...
  const isNew = pathname === "/assistants/create" || id === "create";
  ...
  if (isNew) {
    const locationState = wizardLocationState as {
      templateId?: string;
